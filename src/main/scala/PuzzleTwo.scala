...
object PuzzleTwo {
  ...
  def resolvePosition(moves: List[Move], initialPosition: Position = Position()): Position =
    moves.foldLeft(initialPosition)((pos, move) => pos move move)

  def positionProduct(position: Position): Int = position.horizontal * position.depth
...
object Position {
  ...
  implicit class PositionDsl(position: Position) {
    def move(move: Move): Position = move match {
      case Forward(x) => position
        .focus(_.horizontal).modify(_ + x)
        .focus(_.depth).modify(_ + (position.aim * x))
